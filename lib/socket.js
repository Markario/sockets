...
function setupWorkers(self, options){
	...
	let manualFlush = options.manualFlush !== null ? _.throttle(self.flush.bind(self), options.manualFlush) : function(){};
	...
	function startWorker(id){
		worker(id).then(() => startWorker(id), (err) => { console.error("Error while running handleMessage worker", err); startWorker(id); });
	}
}
