...
function logQueueSize(throttleTime){
	...
	return function* (next){
		...
		throttle(`Queue ${health.queue}`);
		yield next;
	};
}

Socket.prototype.displayQueue = displayQueue;

function logBatches(throttleTime){
	let throttle = _.throttle(function(log){console.log(log);}, throttleTime);
	return function* (next){
		let health = this.health;
		if(this.health.new_batch)
			throttle(`"Batch: ${health.new_batch.numBatch} diff: ${health.new_batch.batchSizeDiff}`);
		yield next;
	};
}

Socket.prototype.displayBatches = displayBatches;
