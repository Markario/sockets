...
function checkHealth(){
	...
	let totalQueued = healthItems.items + healthItems.pendingAdds;

	if((totalQueued > 0 && numQueued === 0)){
		let now = Date.now();
		if(receivedTime && numBatch){
			//Finished a batch
			//calculate how much time we took to process the batch of events
			let diff = now - receivedTime;
			let eventsPerSecond = (numBatch)/(diff/1000);
			healthItems.last_batch = {diff, eventsPerSecond}
		}
		numBatch = totalQueued
		receivedTime = now;
		healthItems.batch = {numBatch, receivedTime};
	}
	numQueued = totalQueued;
	return healthItems;
}
